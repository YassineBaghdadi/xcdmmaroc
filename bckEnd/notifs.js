...
app.get('/', async (req, res) => {
  const [rows] = await db.execute('SELECT * FROM _Notifications where rd = 0 and usr = '+ req.cookies.usdt.id);
  ...
  rows.forEach(e => {
    ...
    htntf += `<a class='dropdown-item preview-item' onclick='ntfClck(${e.id}, &quot;${String(e.link)}&quot;)'> <div class='preview-item-content'> <h6 class='preview-subject font-weight-normal' ${j}>${e.ttle}</h6> <p class='font-weight-light small-text mb-0 text-muted'>${e.msg}</p> </div> </a>`;
  });
  ...
  var [hh] = await db.execute(`select tkn from _Users where id = ${req.cookies.usdt.id}`)
